...
with open('pds_library.txt','w') as lib_out:
    for x in range(0,len(library_file_list),10): # This uses only every 10th structure in the library. At least for two alpha helix fragments using every 10th structure or every 13th structure (very different subset) gave almost identical results => A subset is good enough for the two alpha helix problem. Otherwise might need to be increased.
